...
export interface BusinessOperation<TState, TCommand> {
    execute(runtime: AggregateRuntimeContext<TState>): ExecutionPlan;
}
export interface AggregateLoader {
    load(aggregateId: any): AggregateState;
}
...
export interface AggregateRuntimeContext<TState> {
...
export interface AggregateRuntimeFactory {
    create(command: Command, descriptor: CommandDescriptor): Promise<AggregateRuntimeContext<AggregateState>>;
}
export interface AggregateRebuilder<TState> {
    rebuild(snapshot: Snapshot<TState> | null, events: readonly DomainEvent[]): TState;
}
export interface SnapshotLoader {
    load(aggregateId: any): Snapshot | null;
}
export interface EventLoader {
    load(aggregateId: any, version: any): Event[];
}
export interface AggregateMaterializer {
    materialize(descriptor: CommandDescriptor, command: Command): Promise<AggregateRuntimeContext<AggregateState>>;
}
